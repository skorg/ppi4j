 ...
 *
 * <p>given the code, <code>sub foo : bar(something) {}</code>, the <code>bar(something)</code> part is the attribute.
 * the token represents the entire attribute, as its braces and contents are not parsed into the document tree and are
 * treated by perl (and thusly, by us) as a single string.</p>
 *
...
public class AttributeToken extends Token
{
     ...
     *
     * <p>for the attribute <code>foo(bar)</code>, this method would return <code>bar</code> -for <code>foo()</code>, an
     * empty string is returned. if no attributes are specified, <code>foo</code>, <code>null</code> is returned.</p>
     *
